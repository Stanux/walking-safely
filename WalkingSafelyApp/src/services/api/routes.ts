 ...
 * Requirements: 7.1, 7.2, 7.3, 16.1, 16.2, 16.3, 16.6
 */
...
export const routesService: RoutesService = {
   ...
   * Requirement 7.1, 7.2, 7.3: Calculate route with type preference
   */
  async calculateRoute(request: RouteRequest): Promise<RouteResponse> {
    ...
    const response = await apiClient.post<BackendRouteResponse>(
      ROUTES_ENDPOINTS.CALCULATE,
      {
        ...
        prefer_safe_route: request.preferSafeRoute ?? true, // Default to safest route (Req 7.2)
      },
    );

    console.log('[RoutesService] Raw response:', JSON.stringify(response));

    // Handle the response structure - apiClient returns AxiosResponse
    ...
    const backendOccurrences = (response as any).data?.occurrences || (response as any).occurrences || [];
    ...
    const occurrences = backendOccurrences.map((occ: BackendOccurrence) => ({
      id: String(occ.id),
      location: {
    ...
    // Use instructions from API if available, otherwise create basic ones
    let instructions: RouteInstruction[] = [];
    
    if (route.instructions && route.instructions.length > 0) {
      console.log('[RoutesService] Using instructions from API:', route.instructions.length);
      ...
      console.log('[RoutesService] Creating fallback instructions');
      ...
      if (route.waypoints && route.waypoints.length > 0) {
        route.waypoints.forEach((waypoint: { latitude: number; longitude: number }) => {
          instructions.push({
   ...
   * Requirement 16.1, 16.2: Recalculate when deviation > 30m
   */
